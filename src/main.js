...
(function bootstrap() {
    ...
    window.addEventListener("load", () => {
        window.setTimeout(() => {
            var _a;
            (_a = byId("loader")) === null || _a === void 0 ? void 0 : _a.classList.add("done");
        }, 1900);
    });
    ...
    window.addEventListener("scroll", () => {
        navEl === null || navEl === void 0 ? void 0 : navEl.classList.toggle("scrolled", window.scrollY > 50);
    }, { passive: true });
    // ── 4. CURSOR GLOW ────────────────────────────────────────────
    ...
    // ── 6. MAGNETIC BUTTONS ───────────────────────────────────────
    document
        .querySelectorAll(".btn-y,.btn-gw,.ncta,.btn-dark,.inv-btn,.prog-action a")
        .forEach((btn) => {
    ...
    document
        .querySelectorAll(".vc2,.wyi,.track-card,.why-card,.wc,.speaker-card,.acard,.dcard,.does-card")
        .forEach((card) => {
    ...
    document
        .querySelectorAll(".val-grid,.who-grid,.why-grid,.tracks-grid,.obj-grid,.speakers-grid,.goal-goals")
    ...
    const sectObs = new IntersectionObserver((entries) => {
        entries.forEach((e) => {
            if (e.isIntersecting) {
                ...
                match === null || match === void 0 ? void 0 : match.classList.add("nav-active");
            }
        });
    ...
    hamBtn === null || hamBtn === void 0 ? void 0 : hamBtn.addEventListener("click", toggleMob);
    ...
    const depthItems = document.querySelectorAll(".acard,.dcard,.vc2,.wc,.wyi,.does-card,.mvc");

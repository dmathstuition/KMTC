...
(function premium(): void {
  ...
  if (loader) {
    window.addEventListener("load", (): void => {
      window.setTimeout((): void => loader.classList.add("done"), 1600);
    });
  }
  ...
  document
    .querySelectorAll<HTMLElement>(
      ".btn-y,.btn-gw,.ncta,.btn-dark,.inv-btn,.prog-action a,.mag-btn,.btn-o"
    )
    .forEach((btn): void => {
  ...
  document
    ...
    .forEach((el): void => el.classList.add("shimmer"));
  ...
  if (animItems.length) {
    ...
    window.setTimeout(
      (): void =>
        animItems.forEach((el): void => el.classList.remove("hidden")),
      2000
    );
  }
  ...
  if (navLinks.length) {
      ...
      (entries): void => {
        entries.forEach((e): void => {
          if (e.isIntersecting) {
            ...
            document
              .querySelector<HTMLAnchorElement>(`.nl a[href="#${e.target.id}"]`)
    ...
    document
      .querySelectorAll<HTMLElement>("section[id], div[id]")
      .forEach((s): void => sectObs.observe(s));
  }
  ...
  if (hamBtn) hamBtn.addEventListener("click", toggleMob);
